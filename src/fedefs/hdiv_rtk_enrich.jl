...
function ExtendableFEMBase.get_basis(::Type{ON_CELLS}, ::Type{HDIVRTkENRICH{2, order, true}}, EG::Type{<:Triangle2D}) where {order}
    ...
    #@show chosen_bubbles
    function closure(refbasis, xref)
        refbasisRT0[1,1] = xref[1];    refbasisRT0[1,2] = xref[2]-1
        refbasisRT0[2,1] = xref[1];    refbasisRT0[2,2] = xref[2]
        refbasisRT0[3,1] = xref[1]-1;  refbasisRT0[3,2] = xref[2]
        ...
        if order >= 1
            ...
            refbasis[next,:] = refbasisRT1[1,:]; next += 1
            refbasis[next,:] = refbasisRT1[2,:]; next += 1
        end
        if order >= 2
            # special bubbles with zero lowest order divergence moments
            refbasis[next,:] .= (5*(1-xref[1]-xref[2])-2)*(-refbasisRT1[1,:]-refbasisRT1[2,:]); next += 1
            refbasis[next,:] .= (5*xref[1]-2)*refbasisRT1[2,:]; next += 1
            refbasis[next,:] .= (5*xref[2]-2)*refbasisRT1[1,:]; next += 1
        end
        if order >= 3
            # special bubbles with zero lowest order divergence moments
            refbasis[next,:] .= -(7*(1-xref[1]-xref[2])^2 - 6*(1-xref[1]-xref[2]) + 1)*(-refbasisRT1[1,:]-refbasisRT1[2,:])/7; next += 1
            refbasis[next,:] .= (7*xref[1]^2 - 6*xref[1] + 1)*refbasisRT1[2,:]/7; next += 1
            refbasis[next,:] .= (7*xref[2]^2 - 6*xref[2] + 1)*refbasisRT1[1,:]/7; next += 1
...
function ExtendableFEMBase.get_basis(::Type{ON_CELLS}, ::Type{HDIVRTkENRICH{2, order, true}}, EG::Type{<:Tetrahedron3D}) where {order}
    ...
    function closure(refbasis, xref)
        # RT0
        refbasisRT0[1,1] = 2*xref[1];      refbasisRT0[1,2] = 2*xref[2];      refbasisRT0[1,3] = 2*(xref[3]-1)
        refbasisRT0[2,1] = 2*xref[1];      refbasisRT0[2,2] = 2*(xref[2]-1);  refbasisRT0[2,3] = 2*xref[3]
        refbasisRT0[3,1] = 2*xref[1];      refbasisRT0[3,2] = 2*xref[2];      refbasisRT0[3,3] = 2*xref[3]
        refbasisRT0[4,1] = 2*(xref[1]-1);  refbasisRT0[4,2] = 2*xref[2];      refbasisRT0[4,3] = 2*xref[3]
