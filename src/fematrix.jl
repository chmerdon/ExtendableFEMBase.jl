...
function FEMatrix{TvM,TiM}(FESX::Array{<:FESpace{TvG,TiG},1}, FESY::Array{<:FESpace{TvG,TiG},1}; name = "auto") where {TvM,TiM,TvG,TiG}
    ...
    for j=1:length(FESX)
        offsetY = 0
        for k=1:length(FESY)
            Blocks[(j-1)*length(FESY)+k] = FEMatrixBlock{TvM,TiM,TvG,TiG,eltype(FESX[j]),eltype(FESY[k]),assemblytype(FESX[j]),assemblytype(FESY[k])}(name * " [$j,$k]", FESX[j], FESY[k], offset , offsetY, offset+FESX[j].ndofs, offsetY+FESY[k].ndofs, entries)
